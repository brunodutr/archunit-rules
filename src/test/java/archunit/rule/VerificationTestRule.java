...
public class VerificationTestRule extends ArchCondition<JavaClass> {

	private String message = "Class {0} has unit tests on correct package structure";

	public static VerificationTestRule haveUnitTest() {
		return new VerificationTestRule("have unit testing in the same package structure");
	}

	public VerificationTestRule(String description, Object... args) {
		super(description);
	}
	...
	private boolean verify(JavaClass actualJavaClass) {
		...
		if (isTestClass(actualJavaClass)) {
			...
			boolean hasUnitTest = classes
					.anyMatch(otherJavaClass -> {
	...
	private boolean compareClasses(JavaClass actualJavaClass, JavaClass otherJavaClass, Function<String, String> function) {
